 ...
 * Should be included with variables $prefix, $options, $endpoint, $title (optional)
 */
...
foreach ( $options as $k => $f ) {
	$l = isset( $f['label'] ) ? $f['label'] : str_replace( '_', ' ', $k );
	$k = $prefix . $k;// add namespacing... maybe this isnt the best way
	$v = get_option( $k, '' );
	...
	if ( !empty( $f['callback'] ) && function_exists( __NAMESPACE__ .'\\'. $f['callback'] ) ) {
		...
		switch ( $f['type'] ) {
			case 'textarea':
				echo "<label for='{$k}'>{$l}</label><td><textarea id='{$k}' name='{$k}' placeholder='{$ph}' rows=8 class={$size}-text>{$v}</textarea>";
				break;
			case 'code':
				echo "<label for='{$k}'>{$l}</label><td><textarea id='{$k}' name='{$k}' placeholder='{$ph}' rows=8 class='large-text code'>{$v}</textarea>";
				break;
			case 'number':
				$size = !empty( $f['size'] ) ? $f['size'] : 'small';
				echo "<label for='{$k}'>{$l}</label><td><input id='{$k}' name='{$k}' placeholder='{$ph}' value='{$v}' class={$size}-text type=number>";
				break;
			case 'radio':
				if ( !empty( $f['options'] ) && is_array( $f['options'] ) ) {
					...
					foreach ( $f['options'] as $ov => $ol ) {
						...
						echo "<label><input name='{$k}' value='{$ov}'"; if ( $v == $ov ) echo " checked"; echo " type=radio>{$ol}</label> ";
					}
				}
				break;
			case 'select':
				if ( !empty( $f['options'] ) && is_array( $f['options'] ) ) {
					echo "<label for='{$k}'>{$l}</label><td><select id='{$k}' name='{$k}'>";
				...
				break;
			case 'text':
				echo "<label for='{$k}'>{$l}</label><td><input id='{$k}' name='{$k}' placeholder='{$ph}' value='{$v}' class={$size}-text>";
				break;
			case 'checkbox':
			default:
				echo "<label for='{$k}'>{$l}</label><td><input id='{$k}' name='{$k}'"; if ( $v ) echo " checked"; echo " type=checkbox >";
				break;
		}
	}
